...
* **Backend ({term}`collective.volto.turnstile`):** Manages administrative settings via a {term}`Plone` control panel
  ...
  provides REST API endpoints to validate tokens submitted by the client against Cloudflare's verification API.
...
* The {term}`Secret Key` remains strictly on the backend, shielded from anonymous users to prevent unauthorized access
  or abuse.

* The {term}`Site Key` is exposed to the frontend so the widget can initialize correctly in the browser.



### B. Anonymous Access & Public Exposure Strategy

Because visitors interacting with public forms (like a newsletter subscription or contact page) are unauthenticated (anonymous),
endpoints or settings required for frontend rendering must be carefully handled:
