...
class SettingsControlpanelTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.controlpanel_url = "/@controlpanels/cloudflare-turnstile-settings"
    ...
    def get_record_value(self):
        ...
        record = api.portal.get_registry_record(
            "settings",
            interface=ICloudflareTurnstileSettings,
            default="",
        )
        if not record:
            return {}
    ...
    def test_controlpanel_listed(self):
        """Test that the Cloudflare Turnstile Settings control panel is listed in the control panels overview."""
        response = self.api_session.get("/@controlpanels")

        titles = [x.get("title") for x in response.json()]
        self.assertIn("Cloudflare Turnstile Settings", titles)
